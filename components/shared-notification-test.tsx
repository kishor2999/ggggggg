...
interface SharedNotificationTestProps {
    dashboardType: 'admin' | 'user' | 'employee';
}

export function SharedNotificationTest({ dashboardType }: SharedNotificationTestProps) {
    ...
    const getNotificationMessage = () => {
        const messages = {
            admin: 'New booking needs review',
            user: 'Your booking has been confirmed',
            employee: 'You have been assigned a new task'
        };
        return `${messages[dashboardType]} #${count + 1}`;
    };

    const addNotification = () => {
        const newNotification = {
            id: Date.now().toString(),
            title: `${getNotificationTitle()} ${count + 1}`,
                                ...
                                {isOpen && notifications.length > 0 && (
                                    <div className="absolute right-0 mt-2 w-80 bg-white rounded-md shadow-lg overflow-hidden z-50 border">
                                        <div className="p-2 text-sm font-medium border-b">
                                            Notifications ({dashboardType.charAt(0).toUpperCase() + dashboardType.slice(1)})
                                        </div>
